...
from models import HCGNN_Model, RDesignPro_Model
...
class HCGNN_Design(Base_method):
    ...
    def _build_model(self):
        # return HCGNN_Model(self.args).to(self.device)
        return RDesignPro_Model(self.args).to(self.device)
    ...
    def train_one_epoch(self, train_loader):
        ...
        for batch in train_pbar:
            ...
            # aug_logits, aug_S, aug_graph_prjs = self.model(aug_Xs, oriS[aug_idxs], mask[aug_idxs])
            ...
            # secondary ss
            # if self.args.ss_case == 0:
            #     basic_loss = self.criterion(logits, S).mean()
            ...
            loss = 0.
            basic_loss, aug_loss = 0.,0.
            # for logits in logits_lst:
            #     loss = loss + self.criterion(logits, S).mean()
            # cluster-level contrastive learning
            uni_clus = torch.unique(clus)
            nidxs = torch.zeros_like(clus)
            for c_idx in uni_clus:
            ...
            if self.args.ss_case == 0:
                for logits in logits_lst:
                # loss = loss + self.criterion(logits, S).mean()
                    basic_loss = basic_loss + self.criterion(logits, S).mean()
            elif self.args.ss_case == 1:
                ss_pos = ss_pos[mask == 1].bool()
                basic_loss = (self.criterion(logits[~ss_pos], S[~ss_pos]) + self.criterion(logits[ss_pos] / self.args.ss_temp, S[ss_pos])) / 2.0
            ...
            if self.args.aug_log_case == 0:
                ...
                for aug_logit in aug_logits:
                    aug_loss = aug_loss + self.criterion(aug_logit, aug_S).mean()
                loss = (basic_loss + aug_loss) / (1 + len(aug_logit) / len(logits)) + loss_clu_con * self.args.weigth_clu_con + loss_sam_con * self.args.weigth_sam_con
                # loss = (basic_loss + self.criterion(aug_logits, aug_S)) / (1 + len(aug_logits) / len(logits)) + loss_clu_con * self.args.weigth_clu_con + loss_sam_con * self.args.weigth_sam_con
